"""Tests that chat endpoints require authentication."""
...
def test_chat_requires_authentication() -> None:
    """Test that /chat returns 401 without a valid token."""
...
def test_chat_stream_requires_authentication() -> None:
    """Test that /chat/stream returns 401 without a valid token."""
    with TestClient(app) as client:
        response = client.post(
            "/chat/stream",
            json={"messages": [{"role": "user", "content": "Hello"}]},
...
def test_chat_works_with_valid_token() -> None:
    """Test that /chat works with a valid authentication token."""
    with TestClient(app) as client:
        # Login to get token

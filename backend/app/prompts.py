...
CONTEXT_INSTRUCTION = """下面是从装修知识库检索到的资料。请**优先依据这些资料**回答用户问题；若资料相关，请在回答末尾用一行「依据：……」标注用到的来源名称。若资料不足以支撑回答，就如实说明这超出了已有资料范围，给出审慎的通用建议，并建议咨询专业人士，不要编造具体数字或细节。"""

NO_CONTEXT_NOTE = """（知识库暂未检索到与该问题直接相关的资料。）请基于通用常识审慎回答，并提醒用户：具体数值与做法会因地区、房屋状况、材料档次而不同，重要决策请咨询专业人士或实地确认。不要编造看似精确的数据。"""
